...
export function toggleTodo(todoIndex){
  cy.get(`.todo-list li:nth-chlid(${todoIndex + 1}).toggle`).click()
}

export function validateTodoTxt(todoText, expectedText){
  cy.get(`.todo-list li:nth-child(${todoText + 1}) label`).should('have.text', expectedText)
}
...
export function validateTodoCompletedState(todoIndex){
...
export function validateToggleState(todoIndex, shouldBeToggle){
  const label = cy.get(`.todo-list li:nth-child(${todoIndex +1}) label`)

  label.should(`${shouldBeToggle ? '' : 'not.'}be.checked`)
}